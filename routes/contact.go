...
func (*Contact) GET(w http.ResponseWriter, r *http.Request) {
	...
	render.Page(w, r, Title{"Contact Garrett"},
		...
		components.Margins(Form{Class("relative grid gap-2 rounded-sm border border-slate-500 bg-gray-200 dark:bg-gray-800 p-4 sm:grid-cols-2 md:grid-cols-3 mt-8"),
			...
			Div{
				Label{Class("pl-2 text-slate-700 dark:text-slate-300 text-sm italic"), "Name"},
				Input{Class("block w-full px-2 py-1 rounded-sm border bg-zinc-100 dark:bg-zinc-900 border-slate-500"),
					{"name", "name"}, {"required"}, {"type", "text"}, {"placeholder", name}},
				P{Class("text-sm text-red-600 px-2 empty:hidden"), Attr{"id", "name-error"}},
			},
			Div{Class("md:col-span-2"),
				Label{Class("pl-2 text-slate-700 dark:text-slate-300 text-sm italic"), "Email"},
				Input{Class("block w-full px-2 py-1 rounded-sm border bg-zinc-100 dark:bg-zinc-900 border-slate-500 col-span-2"),
					{"name", "email"}, {"required"}, {"type", "email"}, {"placeholder", email}},
				P{Class("text-sm text-red-600 px-2 empty:hidden"), Attr{"id", "email-error"}},
			},
			Div{Class("col-span-full"),
				Label{Class("pl-2 text-slate-700 dark:text-slate-300 text-sm italic"), "Message"},
				Textarea{Attrs{Class("block w-full px-2 py-1 rounded-sm border bg-zinc-100 dark:bg-zinc-900 border-slate-500"),
			...
			Button{Attrs{Class("absolute -bottom-[7px] right-8 rounded-sm border border-slate-500 bg-zinc-100 dark:bg-zinc-900 px-4 py-1 text-sm hover:bg-slate-200 dark:hover:bg-slate-800"),
				{"type", "submit"}},
				"Send",
			},
		}),
